...
void readCsv(const std::string& strPathGtInfo, std::map<std::string, csvData>& out_csv_data);
struct ExampleParam {
...
int main(int argc, char** argv) {
    ...
    std::cout << "cost time: " << delta << "[ms]" << std::endl;
    return 0;
}
...
void readCsv(const std::string& strPathGtInfo, std::map<std::string, csvData>& out_csv_data) {
    ...
    while (std::getline(file, line)) {
        ...
        while (it != itEnd) {
            std::cout << it->first << " " << it->second.q[0] << " " << it->second.q[1] << " " << it->second.q[2] << " "
