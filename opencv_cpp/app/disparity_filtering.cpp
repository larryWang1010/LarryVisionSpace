...
int main(int argc, char** argv) {
    ...
    if (left.empty() || right.empty()) {
        cout << "Error, Cannot read image file!" << std::endl;
        return -1;
    }
    //! [load_views]
    bool noGT;
    ...
    string imgPathInput = "./data/disparity.png";
    Mat imgInput1 = imread(imgPathInput, cv::IMREAD_ANYDEPTH);
    Mat imgInput2;
    readGT(imgPathInput, imgInput2);  // ? 读取 CV_16S 存储的视差图
    ...
    } else if (filter == "fbs_conf")  // filtering with fbs and confidence using also wls pre-processing
    {
        ...
        fastBilateralSolverFilter(left, left_disp_resized, conf_map / 255.0f, solved_disp, fbs_spatial, fbs_luma,
                                  fbs_chroma, fbs_lambda);
        solving_time = ((double)getTickCount() - solving_time) / getTickFrequency();
        //! [filtering_fbs]

        //! [filtering_wls2fbs]
        fastBilateralSolverFilter(left, filtered_disp, conf_map / 255.0f, solved_filtered_disp, fbs_spatial, fbs_luma,
                                  fbs_chroma, fbs_lambda);
        //! [filtering_wls2fbs]
#else
        (void)fbs_spatial;
        ...
        if (algo == "bm") {
            ...
            matching_time = (double)getTickCount();
            matcher->compute(left_for_matcher, right_for_matcher, left_disp);  // disparity image
            matching_time = ((double)getTickCount() - matching_time) / getTickFrequency();
        } else if (algo == "sgbm") {
    ...
    // *  save
    if (dst_path != "None") {  // 视差滤波后的图
        Mat filtered_disp_vis;
        getDisparityVis(filtered_disp, filtered_disp_vis, vis_mult);
        imwrite(dst_path + ".png", filtered_disp_vis);
    }
    if (dst_raw_path != "None") {  // 视差原图
        Mat raw_disp_vis;
    ...
    if (!no_display) {  // 显示原图
        ...
        cv::Mat disp_raw_color;
        applyColorMap(filtered_disp_vis, disp_raw_color, cv::COLORMAP_JET);
        cv::imshow("Test disp color", disp_raw_color);
        // 数据集中的视差图
        cv::Mat disp_float;
        imgInput1.convertTo(disp_float, CV_32F, 1.0 / 64);
        cv::Mat cvDisparity;
        disp_float.convertTo(cvDisparity, CV_8UC1, 255.0 / (64), 0);
        cv::imshow("Load Disparty Image", cvDisparity);
        cv::Mat disp;
        getDisparityVis(disp_float, disp, vis_mult);  // CV_16S || CV_32F
        cv::imshow("Load Disparity Image2", disp);

        cv::Mat cvDisparityColor;
        applyColorMap(cvDisparity, cvDisparityColor, cv::COLORMAP_JET);
        cv::imshow("Load Disparty Image Color", cvDisparityColor);

        std::cout << "Mat data type test: " << imgInput1.type() << " " << imgInput2.type() << " "
                  << filtered_disp.type() << " " << filtered_disp_vis.type() << std::endl;
