 ...
 *
 ...
 SE(n+1) 特殊欧式群      对应   n*n 的旋转矩阵和 n*1 的平移向量组合成的变换矩阵 群(集合)
 so(n)   对应的李代数 为 so(n)   n×1  列向量   使得向量 和 代数 一一对应  可以使用代数的更新方法来更新矩阵
 ...
 欧拉角定义：
 旋转向量定义的 李群SO(3)   Sophus::SO3 SO3_v( 0, 0, M_PI/2 );  // 亦可从旋转向量构造
 这里注意，不是旋转向量的三个坐标值，有点像欧拉角构造。 旋转向量 转 旋转矩阵  Eigen::Matrix3d R =
 Eigen::AngleAxisd(M_PI/2, Eigen::Vector3d(0,0,1)).toRotationMatrix(); 旋转矩阵定义的 李群SO(3)   Sophus::SO3 SO3_R(R);
 Sophus::SO(3)可以直接从旋转矩阵构造 旋转矩阵 转 Eigen::Quaterniond q(R);            // 或者四元数(从旋转矩阵构造)
